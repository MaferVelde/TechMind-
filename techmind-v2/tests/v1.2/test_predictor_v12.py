...
def test_model_info(predictor: TechMindPredictor) -> None:
    ...
    assert info["confidence_control"]["threshold"] == 0.8132
    assert info["artifact_sha256"] == EXPECTED_SHA256
    assert info["scores_are_probabilities"] is False


def test_predictor_backend_regression(predictor: TechMindPredictor) -> None:
    ...
    assert {entry["category"] for entry in item["top_k"]} == VALID_CLASSES


def test_predictor_explanation(predictor: TechMindPredictor) -> None:
    result = predictor.predict(
        "Train a classifier with cross validation and evaluate precision recall and F1.",
