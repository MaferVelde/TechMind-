...
def test_four_operational_states(
    predictor: TechMindPredictor,
) -> None:
    result = predictor.predict(
        [
            ...
            # review
            (
                "Store application backups in object storage "
                "with lifecycle policies."
            ),

            # rejected_ood
            (
                "Preparar una tortilla de patatas con huevos, "
    ...
    assert (
        predictions[1]["reason"]
        == "low_decision_margin"
    )
    assert (
        predictions[1]["domain_similarity_5nn"]
        >= predictor.domain_threshold
    )
    assert (
        predictions[1]["decision_margin"]
        < predictor.margin_threshold
    )

    assert predictions[2]["decision"] == "rejected_ood"
    assert (
        predictions[2]["reason"]
        == "low_semantic_domain_support"
    )
    assert (
        predictions[2]["domain_similarity_5nn"]
        < predictor.domain_threshold
    )
...
def test_summary_matches_decisions(
    predictor: TechMindPredictor,
) -> None:
    result = predictor.predict(
        [
